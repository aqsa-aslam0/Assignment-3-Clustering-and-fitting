...
def balance_data(df):
  ...
  renewable_energy_consumption = remove_null_values(df[['renewable_energy_consumption']])

  renewable_electricity = remove_null_values(df[['renewable_electricity']])

  co2_emissions = remove_null_values(df[['co2_emissions']])

  min_length = min(len(renewable_energy_consumption), len(renewable_electricity), len(co2_emissions))
  ...
  clean_data = pd.DataFrame({
                                ...
                                'renewable_energy_consumption': [renewable_energy_consumption[x][0] for x in range(min_length)],
                                'renewable_electricity': [renewable_electricity[x][0] for x in range(min_length)],
...
scaler = MinMaxScaler()
scaled_data = scaler.fit_transform(df[['renewable_energy_consumption', 'renewable_electricity',
       'co2_emissions']])
...
plot_clusters('renewable_energy_consumption', 'co2_emissions')

plot_clusters('co2_emissions', 'renewable_electricity')
...
plot_country_cluster('Argentina', 'renewable_energy_consumption', 'co2_emissions')

plot_country_cluster('Somalia', 'renewable_electricity', 'co2_emissions')
